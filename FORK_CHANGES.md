...
### 3. Bootstrap Process - Important!

**For the first release**, chart dependencies use the upstream OpenTelemetry repository:
- `opentelemetry-demo` depends on upstream `opentelemetry-collector`
- `opentelemetry-kube-stack` depends on upstream `opentelemetry-operator`

This is necessary because your Liteverge repository doesn't exist until after the first successful release.

**After your first successful release**, update these dependencies:

1. Edit `charts/opentelemetry-demo/Chart.yaml` - change repository URL to Liteverge
2. Edit `charts/opentelemetry-kube-stack/Chart.yaml` - change repository URL to Liteverge
3. Edit `.github/workflows/release.yaml` - change helm repo add to use Liteverge
...
See `RELEASE_NOTES.md` for detailed instructions.
